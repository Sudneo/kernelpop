...
def write_report_ordered_exploits(ordered_exploits,report_file, header, error_message):
    """
    ...
    if len(ordered_exploits[HIGH_RELIABILITY]) > 0:
        for high_exploit in ordered_exploits[HIGH_RELIABILITY]:
            report.write(high_exploit.name+", HIGH,"+high_exploit.brief_desc+"\n")
    if len(ordered_exploits[MEDIUM_RELIABILITY]) > 0:
        for medium_exploit in ordered_exploits[MEDIUM_RELIABILITY]:
...
def kernelpop(mode="enumerate", uname=None, exploit=None, osx_ver=None, report_file=None):
    """
    kernelpop()
    ...
    if exploit:
        ...
        if report_file:
            write_report_ordered_exploits(identified_exploits["confirmed"],report_file,"Confirmed exploits","no confirmed exploits found")
            write_report_ordered_exploits(identified_exploits["potential"],report_file,"Potential exploits","no potential exploits found")
